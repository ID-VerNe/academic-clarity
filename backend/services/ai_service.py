...
from core.api_key_manager import api_key_manager, KeyState
...
async def call_ocr_api(image, api_config):
    ...
    if api_config.get('_use_multi_key', False):
        max_retries = 10
        for attempt in range(max_retries):
            key_state = await api_key_manager.acquire_key()
            if key_state:
                break
            await asyncio.sleep(0.5)
        
        if not key_state:
            return "OCR Failed: No available API keys"
        ...
        try:
            ...
            await api_key_manager.report_success(key_state, tokens_used)
            return response.choices[0].message.content
        except Exception as e:
            await api_key_manager.report_error(key_state, str(e))
            return f"OCR Failed: {str(e)}"
        finally:
            if key_state:
                await api_key_manager.release_key(key_state)
    else:
        try:
            response = await asyncio.to_thread(
                completion,
...
async def call_chat_api(query, context, api_config):
    ...
    if api_config.get('_use_multi_key', False):
        ...
        if not key_state:
            return "Chat Error: No available API keys"
        ...
        try:
            ...
            await api_key_manager.report_success(key_state)
            return response.choices[0].message.content
        except Exception as e:
            await api_key_manager.report_error(key_state, str(e))
...
async def call_json_extraction_api(md_content, api_config, prompt_instructions):
    ...
    if api_config.get('_use_multi_key', False):
        ...
        if not key_state:
            return json.dumps({"error": "Extraction failed: No available API keys"})
